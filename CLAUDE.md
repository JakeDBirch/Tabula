...
- **LOOP cycles one bar**, not the whole pattern: `patLen` becomes that bar's length and each part's cursor runs `lBar*COLS + step%loopLen`. The bar belongs to whatever is playing — the song's current entry in song mode, the pattern you're editing otherwise.
