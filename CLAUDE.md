...
### Row keys (audition) and note names
...
- **The same toggle also labels the rows in the grid** — the note name repeated four times across each row at low alpha, brighter on the tonic rows. That is the drum grid's existing treatment for its voice names, and it's the right shape here for a reason worth keeping: a label *per cell* would print the same name sixteen times across a row, because every cell in a row is the same pitch. Per-row carries the identical information, and four across leaves ~78px per label on a phone instead of 7px text in a 19.6px box.
- The label div is **absolutely positioned and first in the row**, so it paints over the (static) cells and under the (absolute, later) note rects: it shows on empty cells and a note covers the one it sits on. Both are `position:absolute` with no `z-index`, so **DOM order is what decides** — move the label after the cells map and it would paint over your notes.
