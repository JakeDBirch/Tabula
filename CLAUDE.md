...
- **Parts loop to fill.** A part sounds its own `gridLen` and repeats it for as long as the pattern lasts, so a 1-bar drum part keeps going through a 4-bar pattern. This only works because `resizePatBars` leaves `gridLen` alone when a pattern grows — it used to snap it out to the full new width, which silently killed loop-to-fill and left added bars silent. Writing a note into a bar past a part's end extends that part through that bar (`growLenTo`, called from the synth tap / paint-start / paint-move and `setDrumCell`); erasing never shortens, and the length slider stays the way to trim. DUP BAR extends only the parts whose loop actually covered the duplicated bar.
