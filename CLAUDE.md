...
- **LOOP is a SCOPE: tap loops the BAR, hold (or right-click) loops the whole PATTERN.** `loopMode` is `0` off / `1` bar / `2` pattern — a number rather than a second flag, because every existing read of it is a truthy test (`if(loopMode)`, `loopR.current`, `loopMode?S.loopOn`), so they all kept working and an old save carrying `true` reads as the bar loop it always was. **Nothing new to persist**, which is the one time the multi-site rule has cost nothing. In the core it is the same param, `LL_P_LOOP`, taking the value 2.
