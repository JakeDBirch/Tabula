...
- **The column collapses**, via the `♪` toggle in the bar strip — `rowKeysOpen`, and every offset reads the one derived `rowKeyPad` / `drumKeyPad` so the two states can't drift apart. It collapses to **nothing**, not to a thin stub: the column costs 24px of a 370px phone grid and a stub would hand back half of it, so the handle has to live somewhere that isn't the width you were trying to reclaim. The bar strip is that place — on screen on every part page, drums included, with nothing column-aligned depending on it. Collapsed, the grid is pixel-identical to what it was before row keys existed (21.3px cells on an iPhone 13, against 19.6 with the column showing).
