...
**The song lane is on the PART PAGES too, above the grid** — two rows of it, in **portrait only** (`SONG_STRIP`, `songLane(2)`). The space is free there and only there: the grid is width-bound in portrait (370px of a 390px phone), so vertical room the interface gives back cannot become grid and would otherwise stay empty — this cost the grid nothing on either an iPhone 15 or an SE. Landscape is the opposite, height-bound, where two rows would take ~19% of the layout that already has the smaller grid, so landscape and desktop show no strip and are untouched. **The bar chips move BELOW the grid wherever the strip displaced them**, and stay above where it didn't (`_barStripRow`, one row and two positions, never two copies). That is not a return of "nothing lives under the grid": that rule is about duplicate READOUTS — the step bar and the length track, which said what the grid already said — and a relocated live control is not one. On a phone the bottom is also where your thumb is, and the chips are dragged constantly.

Two things about it are worth knowing before touching it. **One body, two mounts** (`_songLaneBody` / `songLane`): the SONG page passes `null` and the lane takes whatever the mixer leaves; a part page passes a row count and gets a fixed window that scrolls, because the lane grows a row per eight slots and eight rows is taller than any phone has spare. And its **height has to follow its own WIDTH**, because the cells are square and flex-sized (eight across is 41px a cell on a 390px phone, 39px on an SE). No CSS height follows a width — but a **percentage PADDING resolves against the parent's width**, which is the one hook that does, and the arithmetic is linear in W so it lands exactly as `calc(p% - qpx)`: two rows is `calc(25% - 5.5px)`, verified exact at 351/366/406px. The grid's own size term subtracts the same quantity in `vw` for the same reason (`_songStripCss`). The `paddingRight` gutter matters more here than on the song page: the cells set `touch-action:none` so a drag can move a pattern in 2D, which means a touch starting on a cell can never scroll — **that gutter is the only thing you can drag to scroll the window**, and it is why the strip is two rows rather than one.
...
- **A `const` declared inside JSX becomes TEXT, and the build stays green.** Extracting the song lane put `const _songLaneBody=(fixedRows)=>(` inside `songPage`'s markup by one line, and Babel parsed it happily: inside JSX, anything that isn't a tag or a `{}` expression is literal text, so the app would have rendered the words "const _songLaneBody=(fixedRows)=>(" on the song page. No error, no warning. When you move a helper out of a component's JSX, check where it LANDED, not just that the build passed — the same family as the undefined-ref trap below.
