...
class Bell{
  ...
  play(freq,at,sp,noteDur,globalSend,prevFreq,glideTime,layerP,mods){
    ...
    if(spreadAmt>0){
      ...
      // Power-preserving compensation: per-channel power = comp² × (1 + (1-s)²).
      // Solving for power = 2 (the no-spread value) gives this formula.
      // At s=0: comp = 1.0 (no boost). At s=1: comp = √2 ≈ 1.414 (+3 dB).
      spreadComp.gain.value=Math.sqrt(2/(1+(1-spreadAmt)*(1-spreadAmt)));
    ...
    if(p.detune>2&&!isMonoSingle){
      ...
      // Detune widens with spread — 1× at s=0 (matches no-spread sound) up to
      // 2.5× at s=1. With small base detune (e.g. 8 cents), spread alone gave
      // a barely-perceptible stereo image; this multiplier makes it actually
      // sound wide.
      o2.detune.value=p.detune*(1+spreadAmt*1.5);
      panOsc(o2,+spreadAmt);
